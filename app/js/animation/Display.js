...
class Display {
    ...
    updateMask() {
        ...
        switch (this.config.DisplaySettings[prefix + '_shape']) {
            ...
            case 'triangle':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 3});
                break;
            ...
            case 'rect':
                mask = new Mask(this.id, this.canvas, 'rect');
                break;

            case 'quad':
                let m = Math.min(this.canvas.width, this.canvas.height);
                mask = new Mask(this.id, this.canvas, 'rect', {width: m, height: m});
                break;

            case 'pentagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 5});
                break;

            case 'hexagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 6});
                break;

            case 'octagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 8});
                break;

            case 'decagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 10});
                break;

            case 'dodecagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 12});
                break;

            case 'hectagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 16});
                break;

            case 'icosagon':
                mask = new Mask(this.id, this.canvas, 'polygon', {sides: 30});
                break;
        }
    ...
    updateClip() {
        let bounds = this.getBounds();
        this.clip = new Rectangle(bounds.x, bounds.y, bounds.width, bounds.height);
    }
