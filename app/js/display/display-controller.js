...
(function () {
    ...
    HC.DisplayController = function () {
        ...
        var video = {
            _general: {
                ...
                mask_copy: function () {
                    var updates = {};
                    for (var i = 1; i < statics.DisplayValues.display.length; i++) {
                        var v = 'display' + i + '_visible';
                        if (statics.DisplaySettings[v]) {
                            var okey = 'display0_mask';
                            var key = new RegExp('^display' + i + '_mask');
                            for (var c in statics.DisplaySettings) {
                                if (c.match(key)) {
                                    var oc = c.replace(key, okey);
                ...
                mask_reset: function () {
                    ...
                    for (var i = 0; i < statics.DisplayValues.display.length; i++) {
                        var key = new RegExp('^display' + i + '_mask');
                        for (var c in statics.DisplaySettings.initial) {
                            if (c.match(key)) {
     ...
     * @returns {*}
     */
    HC.DisplayController.createMappingControls = function (mapping) {
        var okey = 'display';
        for (var i = 0; i < statics.DisplayValues.display.length; i++) {
            ...
            var _resize = function (key, factor) {
                return function () {
                    var _key = (key + '_' + factor);
                    var data = {};
                    data[_key] = factor;
                    messaging.emitDisplays(data, true, true, false);
                };
     ...
     * @returns {*}
     */
    HC.DisplayController.createControls = function (mapping) {
        var okey = 'display';
        for (var i = 0; i < statics.DisplayValues.display.length; i++) {
     ...
     *
     */
    HC.DisplayController.createAllControls = function () {
