            ...
            fragmentShader: MeshShaderMaterialPlugin.fragmentPrefix + `
                ...
                */
                ...
                float EvalIntensityCurve(vec2 id, float time)
                {
                    ...
                    float uTime = floor(time);
                    float fTime = fract(time);
                    
                    float h = Hash(vec3(id, uTime));
                    //float h = Hash13I(vec3(id, uTime));
                    
                    //if(h < 0.9) h = 0.0;
                    //h = fract(id.x*0.23+id.y*0.1+uTime*0.09);
                    h *= h;
                    h *= h;
                ...
                vec3 EvalTile(vec2 uv, vec2 off, float time)
                {
                    ...
                    float uTime = floor(nTime);
                    float fTime = fract(nTime);
                    
                    float n0 = Hash(vec3(uv, uTime));
                    float n1 = Hash(vec3(uv, uTime + 1.0));
