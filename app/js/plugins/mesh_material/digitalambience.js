{
    HC.plugins.mesh_material.digitalambience = class Plugin extends HC.MeshMaterialPlugin {
        ...
        shader = {
            ...
            {
                vec2 cir = circuit(p);
                return exp(-100.0 * cir.y) + pow(cir.x * 1.8 * (sin(p.z * 10.0 + uTime * -5.0 + cir.x * 10.0) * 0.5 + 0.5), 8.0);
            }
            ...
            void main()
            {
                ...
                vec2 suv = uv;
                uv = 2.0 * uv - 1.0;
                vec3 ro = vec3(0.0, uTime * 0.2, 0.1);
                vec3 rd = normalize(vec3(uv, 0.9));
                ro.xz = rotate(ro.xz, uTime * 0.1);
                ro.xy = rotate(ro.xy, 0.2);
                rd.xz = rotate(rd.xz, uTime * 0.2);
