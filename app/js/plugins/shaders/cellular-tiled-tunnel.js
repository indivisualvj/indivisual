{
    HC.plugins.shaders.cellular_tiled_tunnel = class Plugin extends HC.ShaderPlugin {
        static index = 200;
        static name = 'cellular-tiled-tunnel';
        create() {
            if (!this.pass) {
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ){
	vec2 res = resolution;//vec2(800.0,600.0);

	// Screen coordinates.
	vec2 uv = (fragCoord - res.xy*0.5)/res.y;
	
	// Camera Setup.
	vec3 lookAt = vec3(0.0, 0.0, time*6.);  // "Look At" position.
	...
	if(t < FAR){
	    ...
	    //else sn = doBumpMap(tDiffuse, sp*tSize0, sn, 0.025); // Walls.
        
        sn = doBumpMap(tDiffuse, sp*tSize0, sn, 0.02);
        ...
        vec3 texCol = tex3D(tDiffuse, sp*tSize0, sn);
