...
var Maptastic = function(config) {
    ...
    var original             = getProp(config, 'original', {width: window.innerWidth, height: window.innerHeight});
    ...
    var draw = function() {
        ...
        if(showScreenBounds) {
            ...
            context.beginPath();
            var stepX = canvas.width / 400;
            var stepY = canvas.height / 400;
            ...
            for (var i = 0; i < cx/stepX; i++) {
                context.moveTo(cx + i * stepX, 0);
                context.lineTo(cx + i * stepX, canvas.height);
                context.moveTo(cx-i*stepX, 0);
                context.lineTo(cx-i*stepX, canvas.height);

                context.moveTo(0, cy + i * stepY);
                context.lineTo(canvas.width, cy + i * stepY);
                context.moveTo(0, cy-i*stepY);
                context.lineTo(canvas.width, cy-i*stepY);
            }
        ...
        for(var i = 0; i < layers.length; i++) {
            ...
            if(layers[i].visible){
                ...
                if (showScreenBounds) {
                    ...
                    context.lineWidth = 1;
                    context.moveTo(layers[i].targetPoints[0][0], layers[i].targetPoints[0][1]);
                    context.lineTo(layers[i].targetPoints[2][0], layers[i].targetPoints[3][1]);
                    context.moveTo(layers[i].targetPoints[1][0], layers[i].targetPoints[1][1]);
                    context.lineTo(layers[i].targetPoints[3][0], layers[i].targetPoints[3][1]);
                ...
                centerPoint[0] -= 11 * (layers.length - i);
                centerPoint[1] -= 11 * (layers.length - i);
                ...
                if(showLayerNames) {
                    ...
                    context.fillRect(centerPoint[0] - size[0] / 2, centerPoint[1] - size[1] + 6, size[0], size[1]);
                    context.fillStyle = "black";
                    context.fillText(label, centerPoint[0], centerPoint[1]);

                    if (layers[i] === selectedLayer) {
                        context.fillStyle = "#f22";
                        context.fillRect(size[0] * -.55 + mousePosition[0] - size[0] / 2, size[1] * -.55 + mousePosition[1] - size[1] + 6, size[0], size[1]);
                        context.fillStyle = "black";
                        context.fillText(label, size[0] * -.55 + mousePosition[0], size[1] * -.55 + mousePosition[1]);
                    }
                }
                context.globalAlpha = 1;
            } else {
    ...
    var centerLayer = function (layer, factor, screen, center) {
        ...
        var sps = clonePoints(layer.sourcePoints);

        var lw = (sps[1][0] - sps[0][0]);
        var lh = (sps[2][1] - sps[0][1]);

        if (screen) {
            var lar = lw / lh;
            var sar = window.innerWidth / window.innerHeight;
            if (lar >= sar) { // landscape
        ...
        switch(key){
            ...
            case 72: // h key, flip horizontal.
                if(selectedLayer) {
                    ...
                    swapLayerPoints(selectedLayer.targetPoints, 3, 2);
                    dirty = true;
                    //updateTransform();
                    //draw();
                }
            ...
            case 86: // v key, flip vertical.
                if(selectedLayer) {
                    ...
                    swapLayerPoints(selectedLayer.targetPoints, 1, 2);
                    //updateTransform();
                    //draw();
                    dirty = true;
                }
            ...
            case 82: // r key, rotate 45 degrees.
                if(selectedLayer) {
                    rotateLayer(selectedLayer, Math.PI / 4);
    ...
    var mouseMove = function(event) {
        ...
        var mouseX = translateMouseX(event);
        var mouseY = translateMouseY(event);

        mouseDelta[0] = mouseX - mousePosition[0];
        mouseDelta[1] = mouseY - mousePosition[1];
            ...
            if(selectedPoint) {
                ...
                for (var l = 0; l < layers.length; l++) {
                    ...
                    for (var i = 0; i < cl.targetPoints.length; i++) {
                        cl.targetPoints[i][0] += mouseDelta[0] * scale;
                        cl.targetPoints[i][1] += mouseDelta[1] * scale;
                    }
    ...
    var mouseDown = function(event) {
        ...
        } else if (event.shiftKey && selectedLayer) {
            centerLayer(selectedLayer, 1, false, {x: mouseX, y: mouseY});
            notifyChangeListener(selectedLayer);
            updateTransform();
            draw();
            return false;
        ...
        for(var i = 0; i < layers.length; i++) {
            ...
            for(var p = 0; p < layer.targetPoints.length; p++){
                ...
                if(distanceTo(point[0], point[1], mouseX, mouseY) < selectionRadius) {
                    ...
                    dragging = true;
                    dragOffset[0] = mouseX - point[0];
                    dragOffset[1] = mouseX - point[1];
                    //draw();
                    break;
                }
            }
    ...
    var translateMouseX = function (event) {
        return event.clientX - offsetX;
    };

    var translateMouseY = function (event) {
        return event.clientY - offsetY;
    };
    ...
    var addLayer = function(target, targetPoints, sourcePoints) {
        ...
        for(var n = 0; n < layers.length; n++){
            if(layers[n].element.id == element.id) {
                layers[n].element = element;

                if (sourcePoints) {
                    if (targetPoints) {
                        _copy(targetPoints, layers[n].targetPoints);
                    }
                    _copy(sourcePoints, layers[n].sourcePoints);
        ...
        if (!sourcePoints) {
            ...
            layer.sourcePoints = clonePoints(sourcePoints);
        }
        ...
        if (element.style.zIndex) {
            var zi = parseInt(element.style.zIndex);
            for (var li = 0; li < layers.length; li++) {
                var cl = layers[li];
                var ce = cl.element;
                if (ce.style.zIndex) {
                    var ci = parseInt(ce.style.zIndex);
                    if (ci > zi) {
                        zi = li;
                        break;
                    }
            ...
            layers.splice(Math.min(layers.length, zi), 0, layer);
        } else {
            layers.push(layer);
        }
    ...
    var findLayer = function (element) {
        for(var n = 0; n < layers.length; n++) {
            if (layers[n].element.id == element.id) {
                return layers[n];
            }
    ...
    var loadSettings = function(data) {
        if(data || localStorage.getItem(localStorageKey)){
            if (!data) {
    ...
    return {
        ...
        'addLayer' : function(target, targetPoints, sourcePoints){
            addLayer(target, targetPoints, sourcePoints);
        },
        //'refresh' : function () {
        //    if (getConfigEnabled()) {
        //        draw();
